...
import { FieldPath, collection, doc, documentId, getDoc, getDocs, query, updateDoc, where, arrayRemove, arrayUnion } from "firebase/firestore";
...
export async function PATCH(
    ...
    console.log('req body i addFavorites', req);
    try {
        ...
        try {
            await updateDoc(doc(db, 'favorites', documentId), {
                items: arrayUnion(req.id)
            });
            return NextResponse.json({ message: 'favorite added' }, { status: 200 });
        } catch (error) {
            console.log('error i updatedoc i addfavorite route', error);
            return NextResponse.json({ message: 'Something went wrong in updatedoc i addfavorite route' }, { status: 400 });
        }
        ...
        console.log('error i addfavorite route', error);
        return NextResponse.json({ message: 'Something went wrong in addfavorite route' }, { status: 400 });
    }
}